...
public class BreathWeaponWither extends BreathWeapon {
  ...
  public BreathAffectedBlock affectBlock(World world, Vec3i blockPosition, BreathAffectedBlock currentHitDensity) {
    ...
    if (!world.isRemote) { 
        ...
        entityareaeffectcloud.setParticle(EnumParticleTypes.SPELL_INSTANT);
        ...
        entityareaeffectcloud.addEffect(new PotionEffect(MobEffects.WITHER, 500, 1));
...
//    if (entityID == dragon.getEntityId()) return null;
    ...
    final float DAMAGE_PER_HIT_DENSITY = 0.6F;
...
//    if (currentHitDensity.applyDamageThisTick()) {
          ...
          ((EntityLivingBase) entity).addPotionEffect(new PotionEffect(MobEffects.WITHER, 500, 1));
  //  }
