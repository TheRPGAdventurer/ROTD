...
public class RealmOfTheDragonsConfig {
	...
	public static boolean canFireAffectBlocks = true;
	public static boolean canFireSetFire = true;
	public static boolean netherBreathExplosions = true;
    
	// chances
	public static int MainNestRarity  = 300;
	public static int NestRarerityInX = 64;
	public static int NestRarerityInZ = 64;
	
	public static int netherNestRarity = 700;
	public static int netherNestRarerityInX = 64;
	public static int netherNestRarerityInZ = 64;
	
	public static int undergroundrarityMain = 50;
	public static int undergroundnestX = 64;
	public static int undergroundnestZ = 64;
	
	public static int boneNestMainRarity = 500;
	public static int boneNestRarerityInX = 64;
	public static int boneNestRarerityInZ = 64;
	
	public static double DragonSpeedInGround = 4.7;
	public static double DragonSpeedInAir = 7.7;
	...
	private static void syncconfigs(boolean loadFromConfigFile, boolean readFromConfig) {
		...
		prop.setComment("Determines how fasts dragons fly. Dont exaggerate the numbers. 2 or 3 is fine try decimals like 2.5 or something)");
		...
		prop = config.get(CATEGORY_MAIN, "Can Fire Damage Blocks", canFireAffectBlocks);
		prop.setComment("Enables or Disables dragons ability to damage blocks with fire, turn them to lava etc.");
		canFireAffectBlocks = prop.getBoolean();
		propOrder.add(prop.getName());
		
		prop = config.get(CATEGORY_MAIN, "Can Fire Set Fire on Blocks", canFireSetFire);
		prop.setComment("Enables or Disables dragons ability to set blocks in fire.");
		canFireSetFire = prop.getBoolean();
		propOrder.add(prop.getName());
		
		prop = config.get(CATEGORY_MAIN, "Can Fire Set Fire on Blocks", netherBreathExplosions);
		prop.setComment("Enables or Disables the nether dragon breath's ability to explode.");
		netherBreathExplosions = prop.getBoolean();
		propOrder.add(prop.getName());
		...
		prop = config.get(CATEGORY_WORLDGEN, "1 Nest Rarity X", NestRarerityInX);
		prop.setComment("Determines how rare extreme hill nests will spawn in the X Axis. Higher numbers = higher rarity (in other words  how many blocks for another nest to spawn), "
				+ "(Note: Expermiment on a new world when editing these numbers because it may cause damages to your own worlds)");
		NestRarerityInX = prop.getInt();
		propOrder.add(prop.getName());
		
		prop = config.get(CATEGORY_WORLDGEN, "1 Nest Rarity Z", NestRarerityInZ);
		prop.setComment("Determines how rare extreme hill nests will spawn in the Z Axis. Higher numbers = higher rarity (in other words how many blocks for another nest to spawn), "
				+ "(Note: Expermiment on a new world when editing these numbers because it may cause damages to your own worlds)");
		NestRarerityInZ = prop.getInt();
		propOrder.add(prop.getName());
